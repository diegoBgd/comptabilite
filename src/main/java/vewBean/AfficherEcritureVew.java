 ...
 @ManagedBean
 @ViewScoped
 public class AfficherEcritureVew
...
public boolean isPagination() {
   ...
   private void chargerEntrer() {
     ...
     this.listEncaissement = (new EncaissementModel()).getListEncaissement(this.factory, this.selecetdExercice.getId(), this.dateDebut, this.dateFin, TypeEcriture.entreeFond);
     ...
     for (Encaissement enc : this.listEncaissement) {
       if (enc.getCompteBank() != null) {
         ...
         ecriture = createEcriture(enc.getCompteBank().getCodeCpb(), this.codeJrnl, enc.getPiece(), enc.getCommentaire(), enc.getMontantTTC(), enc.getDateOperation(), 0);
       ...
       if (enc.getRecette() != null) {
         ecriture = createEcriture(enc.getRecette().getCodeCpbl(), this.codeJrnl, enc.getPiece(), enc.getCommentaire(), enc.getMontantTTC(), enc.getDateOperation(), 0);
         this.listeEcriture.add(ecriture);
       } 
     } 
   }
   ...
   private void chargerDepense() {
     ...
     for (Depense dep : this.listDepense) {
       ...
       ecriture = createEcriture(dep.getCharge().getCodeCpbl(), this.codeJrnl, dep.getPiece(), dep.getLibelle(), dep.getMontant(), dep.getDateOperation(), 0);
       this.listeEcriture.add(ecriture);
       if (dep.getTaxe() != null) {
   ...
   private Ecriture createEcriture(String compte, String journal, String piece, String libelle, double montant, Date dateEcr, int typeEntre) {
     Ecriture ecr = null;
