...
public class LoginView implements Serializable {
	...
	public void reloadConfiguration() {
        ...
        if (factory != null && !factory.isClosed()) {
            ...
            this.message = null;  // effacer le message d'erreur précédent
        } else {
            desableLogin = true;
            this.message = "Il y a un problème de connexion à la base de données! "
				...
				if (this.utilisateur != null) {
					...
					if (this.utilisateur.getId() > 0) {
						...
						if (getCodeExercice() == null || getCodeExercice().equals("")) {
							...
							HelperC.afficherMessage("Information", "Il préciser l'exercice!",
									FacesMessage.SEVERITY_ERROR);
	...
	public void redirectionBdd() {
		FacesContext context = FacesContext.getCurrentInstance();
		try {
			context.getExternalContext().redirect("/comptabilite/parametreConnexion.jsf");
		} catch (IOException e) {
	...
	public String getExerCode() {
		return exerCode;
	}

	public void setExerCode(String exerCode) {
		this.exerCode = exerCode;
	}
